...
from safe_rl.algorithms.expected.ppo.model import MLPActorCritic, MLPPenalty
...
def ppo_lagnet(config, actor_critic=MLPActorCritic, ac_kwargs=dict(), use_gymnasium=True, use_cost_indicator=True,
    ...
    run_id = datetime.now().strftime('%Y-%m-%d-%H-%M-') + f'expected-ppo-lagnet-{env_id}'
    ...
    def compute_loss_penalty(data, cost):
        obs = data['obs']
    ...
    def update():
        ...
        for i in range(train_penalty_iters):
            ...
            loss_penalty = compute_loss_penalty(data, cur_cost)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Expected PPO Lagrangian Network')
    parser.add_argument('--config', type=str, default='configs/expected/ppo/ppo_lagnet.yaml', help='Path to the YAML configuration file (relative to project root)')
