...
class LSTMBuffer:
    def __init__(self, obs_space, act_space, hid_dim, size, gamma=0.99, lamda=0.97):
        ...
        self.hin_h_buf = np.zeros([size, hid_dim], dtype=np.float32)
        self.hin_c_buf = np.zeros([size, hid_dim], dtype=np.float32)

        self.ptr, self.path_start_idx, self.max_size = 0, 0, size

    def store(self, obs, act, rew, crew, val, cval, logp, h, c):
        ...
        self.hin_h_buf[self.ptr] = h.squeeze(0).squeeze(0).cpu().numpy()
        self.hin_c_buf[self.ptr] = c.squeeze(0).squeeze(0).cpu().numpy()
        self.ptr += 1
    ...
    def get(self):
        ...
        data = dict(
            ...
            cadv=norm_cadv_buf,
            logp=self.logp_buf,
            hin_h=self.hin_h_buf,
            hin_c=self.hin_c_buf,
        )
