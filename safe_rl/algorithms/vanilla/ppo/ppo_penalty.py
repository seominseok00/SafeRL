...
def ppo_penalty(config, actor_critic=MLPActorCritic, ac_kwargs=dict(), env_lib="safety_gymnasium", use_cost_indicator=True, 
        env_id='SafetyPointGoal1-v0', seed=0, epochs=300, steps_per_epoch=30000, gamma=0.99, lamda=0.97,
        clip_ratio=0.2, target_kl=0.01, penalty_coef=1.0, pi_lr=3e-4, vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, max_ep_len=1000, device=None):
    ...
    rollout_logger = {
        ...
        'EpPenRet': deque(maxlen=episode_per_epoch),
    ...
    if env_lib == "gymnasium":
        ...
        o, _ = env.reset()
    ep_ret, ep_pen_ret, ep_cret, ep_len = 0, 0, 0, 0
    ...
    for epoch in range(epochs):
        for t in range(steps_per_epoch):
            ...
            penalized_r = r - penalty_coef * c
            
            ep_ret += r
            ep_pen_ret += penalized_r
            ep_cret += c
            ep_len += 1

            buf.store(o, a, penalized_r, c, v, vc, logp)
            ...
            if terminal or epoch_ended:
                ...
                if terminal:
                    ...
                    rollout_logger['EpPenRet'].append(ep_pen_ret)
        ...
        epoch_logger.append({
            ...
            'EpPenRet': np.mean(rollout_logger['EpPenRet']),
        ...
        epoch_logger_df.to_csv(os.path.join(run_dir, 'ppo_penalty.csv'), index=False)

        # Save model
        torch.save(ac.state_dict(), os.path.join(run_dir, 'ppo_penalty.pth'))

        # Save best model
        current_return = np.mean(rollout_logger['EpRet'])
        current_pen_ret = np.mean(rollout_logger['EpPenRet'])
        current_cost = np.mean(rollout_logger['EpCost'])

        if current_pen_ret >= best_return:
            best_return = current_pen_ret
            torch.save(ac.state_dict(), os.path.join(run_dir, 'best_ppo_penalty.pth'))

        print('Epoch: {} avg return: {}, avg cost: {}, avg penalty return: {}'.format(epoch, current_return, current_cost, current_pen_ret))
